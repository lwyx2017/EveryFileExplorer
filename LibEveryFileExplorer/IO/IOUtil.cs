...
namespace LibEveryFileExplorer.IO
{
    public class IOUtil
    {
        ...
        public static void WriteU24LE(byte[] Data, int Offset, uint Value)
        {
            Data[Offset] = (byte)(Value & 0xFF);
            Data[Offset + 1] = (byte)((Value >> 8) & 0xFF);
        ...
        {
            int discardedInvalidCharCount;
            return GetBytes(hexString, out discardedInvalidCharCount);
        }

        private static bool IsHexDigit(char c)
        {
            int asciiCodeOfUpperCaseA = Convert.ToInt32('A');
            int asciiCodeOfDigit0 = Convert.ToInt32('0');
            char upperCaseChar = char.ToUpper(c);
            int charAsciiCode = Convert.ToInt32(upperCaseChar);
            bool isUpperCaseHexLetter = (charAsciiCode >= asciiCodeOfUpperCaseA) && (charAsciiCode < asciiCodeOfUpperCaseA + 6);
            bool isDigit = (charAsciiCode >= asciiCodeOfDigit0) && (charAsciiCode < asciiCodeOfDigit0 + 10);
            return isUpperCaseHexLetter || isDigit;
        }
        ...
        private static byte[] GetBytes(string hexString, out int discarded)
        {
            ...
            foreach (char currentChar in hexString)
            {
                if (IsHexDigit(currentChar))
                {
                    validHexChars += currentChar;
                }
                else
                {
                    discarded++;
            ...
            if (validHexChars.Length % 2 != 0)
            {
                discarded++;
                validHexChars = validHexChars.Substring(0, validHexChars.Length - 1);
            }

            int byteArrayLength = validHexChars.Length / 2;
            byte[] resultBytes = new byte[byteArrayLength];
            int currentCharIndex = 0;

            for (int byteIndex = 0; byteIndex < resultBytes.Length; byteIndex++)
            {
                string hexPair = new string(new char[2]
                {
                    validHexChars[currentCharIndex],
                    validHexChars[currentCharIndex + 1]
                });
                resultBytes[byteIndex] = HexToByte(hexPair);
                currentCharIndex += 2;
            }
